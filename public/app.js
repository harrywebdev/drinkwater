...
async function init() {
    ...
    if (!('serviceWorker' in navigator) || !('PushManager' in window)) {
        showStatus('Push notifications are not supported in your browser.', 'error');
...
async function subscribe() {
    try {
        ...
        showStatus('Successfully subscribed! You will receive hourly reminders from 8 AM to 8 PM. 🎉', 'success');
        await updateUI();
...
async function unsubscribe() {
    try {
        ...
        showStatus('Successfully unsubscribed. You will no longer receive reminders.', 'info');
        await updateUI();
