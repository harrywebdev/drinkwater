...
const VAPID_PUBLIC_KEY = process.env.VAPID_PUBLIC_KEY || 'BNxSB5ap-W6FWLqEpUqkF7FcF2JZF-P0VHh9D9D0wR2uJhL6HXm7LnqJ5BH-gKqQ9VCl0xMZqNZMZ9ZqZ0Z0Z0Z';
const VAPID_PRIVATE_KEY = process.env.VAPID_PRIVATE_KEY || 'your-private-key-here';

// If no VAPID keys are set, generate new ones
if (VAPID_PUBLIC_KEY === 'BNxSB5ap-W6FWLqEpUqkF7FcF2JZF-P0VHh9D9D0wR2uJhL6HXm7LnqJ5BH-gKqQ9VCl0xMZqNZMZ9ZqZ0Z0Z0Z') {
...
app.post('/api/subscribe', (req, res) => {
  try {
    ...
    if (!subscription || !timezone) {
      return res.status(400).json({ error: 'Missing subscription or timezone' });
    }
...
cron.schedule('* * * * *', async () => {
  ...
  console.log(`Checking notifications... (${subscriptions.size} subscriptions)`);
  ...
  for (const [id, subscriptionData] of subscriptions) {
    ...
    if (shouldNotify) {
      ...
      if (!wasNotificationSentThisHour(lastNotificationSent, targetHour, timezone)) {
        // Random chance to send within the window (creates randomness)
        // Higher chance as we progress through the window
        const randomChance = Math.random();
        if (randomChance < 0.1) { // 10% chance per minute = spread across ~10 minutes
